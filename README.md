...
1. **Headings**: Used to structure the document. For example, `#` for main headers like "RBAC UI", `##` for sections like "Setup Instructions", etc.
2. **Code blocks**: Used for commands like `npm install`, `npm start`, etc. These are enclosed in triple backticks (```) to highlight them as code.
3. **Lists**: Used to format steps and features. Unordered lists (`-`) are used for general points, while ordered lists (`1.`, `2.`, etc.) are used for step-by-step instructions.
4. **Inline code**: Used for specific commands or terms like `VITE_SERVER_URL` to make them stand out.

With this, the README provides a clean, readable, and structured guide for setting up and understanding the RBAC UI project.
